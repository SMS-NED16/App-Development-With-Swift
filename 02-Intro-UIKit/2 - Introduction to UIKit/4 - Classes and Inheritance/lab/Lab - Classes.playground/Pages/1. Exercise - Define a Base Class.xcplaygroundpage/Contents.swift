...
class Spaceship {
    // INSTANCE VARIABLE
    var name: String = ""               // should be var so that it can be chagned
    var health: Double = 0 {
        willSet {
            if newValue <= 0 {
                print("Your ship was hit one too many times! Do you want to play again?")
            }
        }
    }
    var position: Int = 0               // negative = left, positive = right, default = 0
    
    // INSTANCE METHODS
    func moveLeft() {
        position -= 1                   // move one unit to the left
    }
    
    func moveRight() {
        position += 1                   // move one unit to the right
    }
    
    func wasHit() {
        health -= 5                     // decrement by 5
    }
    // Need to provide a member-wise initializer if we want to set the values of the instance variables
}

/*:
 Create a `let` constant called `falcon` and assign it to an instance of `Spaceship`. After initialization, set `name` to "Falcon".
 */
let falcon = Spaceship()        // will be set to default values
falcon.name = "Falcon"
/*:
 ...
 */
falcon.moveLeft(); print(falcon.position);
falcon.moveLeft(); print(falcon.position);      // Move left twice
falcon.moveRight(); print(falcon.position)      // Move right once
/*:
 ...
 */
falcon.health = 10
falcon.wasHit(); print(falcon.health);
falcon.wasHit(); print(falcon.health);
