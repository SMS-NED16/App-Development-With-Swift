 ...
 */
/* Falcon is an instance of class `Spaceship`, which is the superclass of `Fighter`, the class with the `weapon` instance variable. This means that
   even though `Fighter` inhertis all the properties and methods from `Spaceship`, the reverse is not true. An instance of `Spaceship` such as `Falcon`
   does not have a `weapon` property because the `weapon` property is specific to the subclass and the superclass does not inherit properties from the subclass.
 */
 ...
 */
destroyer.fire(); print(destroyer.remainingFirePower)
destroyer.fire(); print(destroyer.remainingFirePower)
destroyer.fire(); print(destroyer.remainingFirePower)
