 ...
 */
let nameInCaps = "Saad Mashkoor Siddiqui"
let name = "saad mashkoor siddiqui"
print(nameInCaps.lowercased() == name ? "The two strings are equal." : "The two strings are not equal.")
/*:
 ...
 */
var comparisonStr = "\(nameInCaps) and \(name)"
comparisonStr += nameInCaps.lowercased() == name ? " are the same." : " are not the same."
print(comparisonStr)
/*:
...
print(junior.hasSuffix("Jr.") ? "We found a second generation name!" : "We did not find a second generation name.")
...
// Must cast both strings to lowercase to ensure no mismatch
