...
func checkAge(_ userAgeInput: String) {
    if let userAge = Int(userAgeInput) {
        print("\(userAge > 18 ? "Welcome!" : "Sorry, but you aren't old enough to use our app.")")
    } else {
 ...
 */
func getUserAge(_ userAgeInput: String) -> Int? {
    if let userAge = Int(userAgeInput) {
        print("\(userAge > 18 ? "Welcome!" : "Sorry, but you aren't old enough to use our app.")")
        return userAge
    } else {
...
func getPrice(itemName: String) -> Double? {
        if let itemCount = stock[itemName] {
        if itemCount != 0 {
...
print(getPrice(itemName: "Chips"))         // Not a good idea to force unwrap it. Could be nil.
print(getPrice(itemName: "Rhubarb"))
