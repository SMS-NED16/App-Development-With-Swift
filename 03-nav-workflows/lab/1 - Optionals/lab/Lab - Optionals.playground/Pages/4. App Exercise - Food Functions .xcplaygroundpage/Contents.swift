...
func getMeal(mealName: String) -> Meal? {
    if let mealInLog = meals[mealName] {
        return mealInLog
    } else {
        return nil
    }
...
print(getMeal(mealName: "Breakfast")!)          // nil
print(getMeal(mealName: "Lunch"))              // nil
/*:
...
func getMealLog() -> [String: Any] {
    if let mealLog = UserDefaults.standard.dictionary(forKey: "mealLog") {
        ...
        return [String: Any]()      // empty dictionary
    }
}

print(getMealLog())     // Empty dictionary - not running on a device, no data in emulated device
